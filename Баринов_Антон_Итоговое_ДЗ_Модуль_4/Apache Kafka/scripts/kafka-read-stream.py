...
def main():
    try:
        ...
        query = df_flat.writeStream \
            ...
            .format('csv') \
            .option('path', 's3a://dataproc-bucket/processed_data_stream/') \
            .option('header', 'true') \
            .option('checkpointLocation', 's3a://dataproc-bucket/checkpoints/') \
            .start()
        ...
        try:
            ...
            error_df = spark.createDataFrame([(error_msg,)], ["error"])
            error_df.coalesce(1).write \
                .mode('overwrite') \
                .format('text') \
                .save('s3a://dataproc-bucket/error_logs/')
        except:
            pass
        sys.exit(1)
